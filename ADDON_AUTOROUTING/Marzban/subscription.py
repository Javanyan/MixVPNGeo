...
#   MIXVPN_ROUTING_SOURCE  = default | jsonsub | whitelist | custom
#   MIXVPN_ROUTING_CUSTOM  = <your happ:// URL>
# ─────────────────────────────────────────────────────────────────────────────────

_MIXVPN_URLS = {
    "default": "https://raw.githubusercontent.com/Javanyan/MixVPNGeo/main/HAPP/DEFAULT.DEEPLINK",
    "jsonsub": "https://raw.githubusercontent.com/Javanyan/MixVPNGeo/main/HAPP/JSONSUB.DEEPLINK",
    "whitelist": "https://raw.githubusercontent.com/Javanyan/MixVPNGeo/main/HAPP/WHITELIST.DEEPLINK",
}


class _MixVPNResolver:
    def __init__(self, default_source: str):
        self._lock = threading.Lock()
        self._value = ""
        self._fetched_at = 0.0
        self._last_fail = 0.0
        self._source = os.environ.get("MIXVPN_ROUTING_SOURCE", default_source).strip().lower()
        self._custom = os.environ.get("MIXVPN_ROUTING_CUSTOM", "").strip()

    def get(self) -> str:
        if self._source == "custom":
            return self._custom
        url = _MIXVPN_URLS.get(self._source)
        if not url:
            return self._custom
        now = _time.monotonic()
...
mixvpn_resolver = _MixVPNResolver("default")
...
def user_subscription(
    ...
    # MixVPN: cached routing deeplink (no blocking HEAD per request)
    _routing = mixvpn_resolver.get()
    if _routing:
