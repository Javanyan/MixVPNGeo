...
curl -fLo /var/lib/marzneshin/subscription.py \
  https://raw.githubusercontent.com/Javanyan/MixVPNGeo/main/ADDON_AUTOROUTING/Marzneshin/subscription.py
```
...
```yaml
    environment:
      MIXVPN_ROUTING_SOURCE: "default"   # default | jsonsub | whitelist | custom
      # MIXVPN_ROUTING_CUSTOM: "happ://..."  # только при source=custom
```
...
| `custom` | Ваша произвольная ссылка из `MIXVPN_ROUTING_CUSTOM` |
...
Готово! Happ-клиенты при следующей синхронизации подписки получат MixVPN-роутинг в заголовке `Routing:`.

## Как это работает

- `subscription.py` монтируется поверх оригинального файла Marzneshin.
- При отдаче подписки он подтягивает содержимое `.DEEPLINK`-файла из [mixvpn-routing](https://github.com/Javanyan/MixVPNGeo) с **кешем 10 минут** — никаких блокирующих запросов на каждый ответ.
